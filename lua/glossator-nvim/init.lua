...
---@field toolbar_hl    { [string]: vim.api.keyset.highlight }
---@field fmt_actions   GlossatorAction[]
---@field par_actions   GlossatorAction[]
---@field tag_to_group  { [string]: string }
...
local function set_highlights()
  for _, t in ipairs(config.hl_tags) do
    api.nvim_set_hl(0, t.group, t.hl)
  end
  for _, t in ipairs(config.ul_tags) do
    api.nvim_set_hl(0, t.group, t.hl)
  end
  for group, hl in pairs(config.toolbar_hl) do
...
local function build_resolve()
  ...
  config.resolve = function(filepath)
    if fn.isdirectory(notes_dir) == 0 then
      fn.mkdir(notes_dir, "p")
    end
    local safe_name = filepath:gsub("[^%w%.]", "_")
    return notes_dir .. "/" .. safe_name .. ".notes.md"
  end
end

-- ── 2. Highlight rendering ────────────────────────────────────────────────────

---Highlight all tags and annotations in the current buffer.
function M.load_highlights()
  ...
  while true do
    ...
    local function highlight_range(group)
      ...
      if oel == cl then
        api.nvim_buf_set_extmark(bufnr, ns_toolbar, oel, oec, {
          end_line = cl,
          end_col = cc,
          hl_group = group,
          priority = 200,
        ...
        for l = oel + 1, cl - 1 do
          api.nvim_buf_set_extmark(bufnr, ns_toolbar, l, 0, {
            end_line = l,
            end_col = #lines[l + 1],
            hl_group = group,
            priority = 200,
        ...
        api.nvim_buf_set_extmark(bufnr, ns_toolbar, cl, 0, {
          end_line = cl,
          end_col = cc,
          hl_group = group,
          priority = 200,
      ...
      api.nvim_buf_set_extmark(bufnr, ns_toolbar, ol, oc, {
        end_line = oel,
        end_col = oec,
        conceal = "",
        priority = 201,
      })
      api.nvim_buf_set_extmark(bufnr, ns_toolbar, cl, cc, {
        end_line = cel,
        end_col = cec,
        conceal = "",
        priority = 201,
    ...
    if is_anno then
      highlight_range("ETAnnotate")
    else
      local group = config.tag_to_group[tag_content]
      if group then
        highlight_range(group)
      end
    end
    pos = ce + 1
  end
  ...
  while true do
    ...
    api.nvim_buf_set_extmark(bufnr, ns_toolbar, sl, sc, {
      end_line = el,
      end_col = ec,
      hl_group = "ETComment",
      priority = 190,
...
local function apply_tag(tag)
  ...
  if ec == 2147483647 then
    local line = api.nvim_buf_get_lines(bufnr, el - 1, el, false)[1]
    ec = #line
  end
  ...
  if sl == el then
    ...
    ec = math.min(ec, #line)
    api.nvim_buf_set_lines(bufnr, sl - 1, sl, false, {
      line:sub(1, sc - 1) .. tag .. line:sub(sc, ec) .. tag .. line:sub(ec + 1),
    })
  else
...
local function apply_annotate()
  ...
  vim.ui.input({ prompt = "Annotation (to Glossator): " }, function(input)
    ...
    if cs_state.notes_buf and api.nvim_buf_is_valid(cs_state.notes_buf) then
      ...
      local current_note_line = api.nvim_buf_get_lines(
        cs_state.notes_buf,
        target_line_idx,
        target_line_idx + 1,
        false
      ...
      api.nvim_buf_set_lines(cs_state.notes_buf, target_line_idx, target_line_idx + 1, false, {
        new_note_line,
      })
    end
    vim.defer_fn(M.load_highlights, 10)
...
local function apply_wrap(o, c)
  ...
  if sl == el then
    ...
    ec = math.min(ec, #line)
    api.nvim_buf_set_lines(bufnr, sl - 1, sl, false, {
      line:sub(1, sc - 1) .. o .. line:sub(sc, ec) .. c .. line:sub(ec + 1),
    })
  else
    local ls = api.nvim_buf_get_lines(bufnr, sl - 1, el, false)
    ec = math.min(ec, #ls[#ls])
...
---Open the formatting/highlight toolbar in a floating window.
function M.open_toolbar()
  ...
  add_section("FMT", config.fmt_actions, false, false)
  l1 = l1 .. sep
  l2 = l2 .. sep
  add_section("WRAP", config.par_actions, false, false)
  l1 = l1 .. sep
  l2 = l2 .. sep
  add_section("HIGHLIGHT", config.hl_tags, true, false)
  l1 = l1 .. sep
  l2 = l2 .. sep
  add_section("UNDERLINE", config.ul_tags, false, true)
  l1 = l1 .. sep
  l2 = l2 .. sep
  ...
  local all_actions = {}
  for _, t in ipairs(config.fmt_actions) do
    table.insert(all_actions, { k = t.key, f = function()
      apply_wrap(t.wrap[1], t.wrap[2])
    end })
  end
  for _, t in ipairs(config.par_actions) do
    table.insert(all_actions, { k = t.key, f = function()
      apply_wrap(t.wrap[1], t.wrap[2])
    end })
  end
  ...
  for _, action in ipairs(all_actions) do
    vim.keymap.set("n", action.k, function()
      close_and_run(action.f)
    end, opts)
  end
  vim.keymap.set("n", "a", function()
    close_and_run(apply_annotate)
  end, opts)
  vim.keymap.set("n", "d", function()
    close_and_run(strip_all_marks)
  end, opts)
...
-- ── 5. Glossator pane: sync logic ─────────────────────────────────────────────

local function is_valid_sync_context()
  local curr_win = api.nvim_get_current_win()
  local curr_buf = api.nvim_get_current_buf()
  if curr_win == cs_state.main_win and curr_buf == cs_state.main_buf then
    return true
  end
  if curr_win == cs_state.notes_win and curr_buf == cs_state.notes_buf then
    return true
  end
  return false
end
...
local function sync_notes_reorder()
  ...
  if #new_notes_lines > 0 then
    ...
    if #new_notes_lines > current_len then
      local empty = {}
      for _ = 1, (#new_notes_lines - current_len) do
        table.insert(empty, "")
      end
...
local function sync_cursor()
  if cs_state.syncing then
    return
  end
  if not is_valid_sync_context() then
    return
  end
  ...
  if target_win and api.nvim_win_is_valid(target_win) then
    if
      api.nvim_win_get_buf(target_win) ~= (
        target_win == cs_state.main_win and cs_state.main_buf or cs_state.notes_buf
      )
    then
      return
    end
...
local function sync_scroll()
  if not api.nvim_win_is_valid(cs_state.main_win) or not api.nvim_win_is_valid(cs_state.notes_win) then
    return
  end
  if not is_valid_sync_context() then
    return
  ...
  if target_buf ~= expected_target_buf then
    return
  end
...
local function adjust_length_on_edit()
  if cs_state.syncing or not api.nvim_buf_is_valid(cs_state.main_buf) then
    return
  end
  if api.nvim_buf_get_lines(cs_state.notes_buf, 0, 1, false) == nil then
    return
  end
...
local function init()
  ...
  vim.api.nvim_create_autocmd(
    { "BufWritePost", "TextChanged", "TextChangedI", "BufEnter", "InsertLeave" },
    { pattern = "*.md", callback = M.load_highlights }
  )
end

---Override default configuration. Must be called before the plugin is first used
---for all options to take effect. Safe to call at any time; already-initialized
---state is re-applied with the new values.
---
---@param opts? GlossatorConfig
function M.setup(opts)
  if not config then
    -- first init with user opts
    ...
    -- custom resolve overrides derived resolve
