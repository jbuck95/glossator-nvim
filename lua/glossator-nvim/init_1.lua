...
local ns_toolbar = api.nvim_create_namespace("editing_toolbar")
...
local ul_tags = {
  ...
  -- Generische Gruppe für Annotation-IDs [a1], [a2]...
...
-- Vorwärtsdeklaration für ComSync State
local cs_state = {
  main_buf = nil,
  notes_buf = nil,
  main_win = nil,
...
local function set_highlights()
  ...
  -- Toolbar Farben
  api.nvim_set_hl(0, "ETHeader",  { fg = "#7f849c", bold = true })
  api.nvim_set_hl(0, "ETKey",     { fg = "#cdd6f4", bold = true })
  api.nvim_set_hl(0, "ETSep",     { fg = "#45475a" })
  -- Annotation ID (versteckt oder subtil)
  api.nvim_set_hl(0, "ETAnnotateID", { fg = "#00ffff", bold = true })
  -- Kommentar Farbe
  api.nvim_set_hl(0, "ETComment", { fg = "#6c7086", italic = true })
end
...
function M.load_highlights()
  ...
  local pos = 1
  while true do
    -- Suche nach Standard-Tags
    local s1, e1 = text:find("%[[hu][rgybp]%]", pos)
    
    -- Suche nach Annotation IDs: [a + Ziffern + ]
  ...
  -- Kommentare highlighten (Fixed Pattern)
  local c_pos = 1
  while true do
...
-- Hilfsfunktion: Nächste freie ID ermitteln
...
local function apply_annotate()
  ...
  vim.ui.input({ prompt = "Annotation (to ComSync): " }, function(input)
    ...
    -- Notes Buffer: Einfügen (Initial an gleicher Zeile)
...
-- ── 3. ComSync: Definitionen & State ──────────────────────────────────────────

local DATA_DIR = fn.expand("~/Documents/comsync")
local ns_comsync = api.nvim_create_namespace("ComSyncHeaderTracking")
...
local function align_by_headers()
  ...
  api.nvim_buf_clear_namespace(cs_state.notes_buf, ns_comsync, 0, -1)
  ...
  for i, line in ipairs(notes_lines) do
    if line:match("^#+%s+") then
      local id = api.nvim_buf_set_extmark(cs_state.notes_buf, ns_comsync, i - 1, 0, {})
  ...
  for i, line in ipairs(main_lines) do
    if line:match("^#+%s+") then
      ...
      if mark_id then
        local mark_pos = api.nvim_buf_get_extmark_by_id(cs_state.notes_buf, ns_comsync, mark_id, {})
...
-- NEU: Reorder Logic basierend auf IDs (MIT DEDUPLIZIERUNG)
...
function M.open_comsync()
  local current_file = api.nvim_buf_get_name(0)
  if current_file == "" then
    vim.notify("ComSync: Cannot sync an unnamed buffer.", vim.log.levels.WARN)
    return
  end
  ...
  local group = api.nvim_create_augroup("ComSync_Session", { clear = true })
  ...
  api.nvim_create_autocmd("BufWritePost", {
    ...
    buffer = cs_state.main_buf,
    callback = function()
      adjust_length_on_edit()   -- 1. Länge sicherstellen
      align_by_headers()        -- 2. Grobe Struktur (Header)
      sync_notes_reorder()      -- 3. Feine Struktur (IDs Reorder)
    end
  })
